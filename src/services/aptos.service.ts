...
export async function handleStakeCoins(account: Account, amount: number) {
    ...
    const txn = await aptos.transaction.build.simple({
        ...
        data: {
            function: `${stakeModuleAddress}::kryzel_staking_v4::stake`,
...
export async function handleGetUserStakes(account: Account) {
    try {
        ...
        const data = {
            function: `${stakeModuleAddress}::kryzel_staking_v4::get_user_stakes`,
            typeArguments: [],
...
export async function handleRestakeFunc(account: Account, index: any) {
    ...
    const txn = await aptos.transaction.build.simple({
        sender: account.accountAddress,
        data: {
            function: `${stakeModuleAddress}::kryzel_staking_v4::restake`,
            typeArguments: [],
            functionArguments: [
                index
            ],
...
export async function handleGetUserDailyReturn(account: Account) {
    try {
        const userAddress = account.accountAddress;
        const data = {
            function: `${stakeModuleAddress}::kryzel_staking_v4::get_user_daily_summary`,
            typeArguments: [],
            functionArguments: [userAddress]
        };
